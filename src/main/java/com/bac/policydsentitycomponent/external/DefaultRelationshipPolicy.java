...
public class DefaultRelationshipPolicy implements RelationshipPolicy, Serializable {
    ...
    private void buildMap(List<? extends ClassEntityComponent> policyComponents) {
        ...
        for (ClassEntityComponent entityComponent : policyComponents) {

//            ClassEntityComponent classEntityComponent = (ClassEntityComponent) entityComponent;
            this.policyComponents.add(entityComponent);
            Map<DataType, NodeRelationship> relationships = new HashMap<>();
            for (EntityComponent component : entityComponent.getEntityComponents()) {

                if (component instanceof ClassEntityComponent) {
                    ClassEntityComponent childClassEntityComponent = (ClassEntityComponent) component;
                    relationships.put(childClassEntityComponent.getPolicyDataType(), component.getRelationshipType());
                } else {
                    relationships.put(component.getDataType(), component.getRelationshipType());
                }
            }
            relationshipMapping.put(entityComponent.getPolicyDataType(), relationships);
        }
    }
    ...
    public boolean isGroupDataType(DataType dataType) {
        ...
        if (dataTypeResolver != null) {
            ...
            if (dataType == classDataType) {    // note equivalence as they are assumed to be Enums
                return true;
            }
        }
        if (dataType == null || relationshipMapping == null || !relationshipMapping.containsKey(dataType)) {
    ...
    public boolean canAssign(DataType from, DataType to) {
        ...
        //  'To' datatype is not part of a dependency relationship with another datatype
