...
public class DataAccessor extends AbstractDataAccessor<PolicyDSEntity> {
	...
	//
	// Override the root node list build. The default behaviour requires that
	// only components that have no hierarchy relationships
	// are returned. For policy components this means that items which have a
	// hierarchy relationship will not be supplied. If
	// the datatype is a Class component datatype then override this behaviour.
	//

	@Override
	protected List<PolicyDSEntity> buildRootNodesList(DataType datatype,
